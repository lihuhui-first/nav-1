...
const simplifyUrl = (url1)=>{
    return url1.replace("https://", "").replace("http://", "").replace("www.", "").replace(/\/.*/, "") //转义字符，删除/开头的内容
    ;
};
const render = ()=>{
...
;
$(".addButton").on("click", ()=>{
    let url1 = window.prompt("请输入添加的网址");
    if (url1.indexOf("http") !== 0) url1 = "https://" + url1;
    console.log(url1);
    hashMap.push({
        logo: simplifyUrl(url1)[0].toUpperCase(),
        logoType: "text",
        url: url1
    });
    render();
...
//# sourceMappingURL=index.3aa542f8.js.map
